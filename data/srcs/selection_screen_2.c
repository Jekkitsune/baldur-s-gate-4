...
/*   Updated: 2025/01/30 09:46:42 by gmassoni         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    check_click_selection_screen(t_data *data)
{
    ...
    if (check_click_character(data, vec(data->win_size.x * 0.1, data->win_size.y * 0.1), "barbarian", size))
		return ;
	if (check_click_character(data, vec(data->win_size.x * 0.4, data->win_size.y * 0.55), "rogue", size))
		return ;
	if (check_click_character(data, vec(data->win_size.x * 0.4, data->win_size.y * 0.1), "warlock", size))
		return ;
	if (check_click_character(data, vec(data->win_size.x * 0.1, data->win_size.y * 0.55), "wizard", size))
		return ;
	if (check_click_character(data, vec(data->win_size.x * 0.7, data->win_size.y * 0.1), "monk", size))
		return ;
	if (check_click_character(data, vec(data->win_size.x * 0.7, data->win_size.y * 0.55), "ranger", size))
		return ;
	check_click_start(data, vec(data->win_size.x * 0.92, data->win_size.y * 0.03), vec(data->win_size.x * 0.06, data->win_size.y * 0.06));
}
