...
/*   Updated: 2025/01/14 23:49:31 by fparis           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_radius_team(t_data *data, int team, t_spellinfo *info)
{
	...
	{
		i2 = info->pos.x - (info->radius - i);
		while (i2 < info->pos.x + (info->radius - i))
		{
			if (in_bound(data->current_map, vec(i2, info->pos.y + i)))
			{
				check_cell_team(&data->current_map->arr[i2][info->pos.y + i], team, &info->nb);
				if (info->nb < 0)
					return ;
			}
...
void	try_all_spell_range(t_data *data, int team, t_spellinfo *info)
{
	t_vector	i;
	t_vector	pos;
	t_spellinfo current;
	...
	while (i.x < info->range + 1)
	{
		...
		while (i.y < info->pos.x + (info->range - i.x))
		{
			...
			if (in_bound(data->current_map, pos) && !has_obstacle_pos(data, info->pos, pos))
			{
				current.nb = 0;
				current.pos = pos;
				check_radius_team(data, team, &current);
			...
			pos = vec(i.y, info->pos.y - i.x);
			if (i.x != 0 && in_bound(data->current_map, pos) && !has_obstacle_pos(data, info->pos, pos))
			{
				current.nb = 0;
				current.pos = pos;
				check_radius_team(data, team, &current);
...
t_bool	get_best_spell_pos(t_data *data, t_spellinfo *spell, t_entity *caster)
{
	...
	try_all_spell_range(data, caster->sheet.team, &info);
	if (info.nb >= goal)
	{
		spell->pos = info.pos;
		return (true);
