...
/*   Updated: 2025/01/16 22:16:56 by fparis           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	mouse_down(t_data *data, int key)
{
	...
	{
		mlx_mouse_get_pos(data->mlx, &data->player.mouse_pos.x,
			&data->player.mouse_pos.y);
		mlx_mouse_hide();
	...
	{
		mlx_mouse_move(data->mlx, data->win, data->player.mouse_pos.x,
			data->player.mouse_pos.y);
		mlx_mouse_show();
	}
}
...
void	update_mouse_pos(t_data *data, t_vector mouse_pos)
{
	if (mouse_pos.x != data->win_size.x / 2)
	{
		mouse_pos.x = (data->win_size.x / 2) - mouse_pos.x;
		data->player.angle = angle_add(data->player.angle,
				-(float)mouse_pos.x / 1000 * 1.2);
	}
	if (mouse_pos.y != data->win_size.y / 2)
	{
		...
		if (data->player.focus_mode)
		{
			if ((mouse_pos.y < 0 && data->player.height < HEIGHT_CAP)
				|| (mouse_pos.y > 0 && data->player.height > -HEIGHT_CAP))
				data->player.height -= mouse_pos.y * 10;
			if ((mouse_pos.y < 0 && data->player.pitch > -75)
				|| (mouse_pos.y > 0 && data->player.pitch < 75))
...
void	camera_move(t_data *data)
{
	...
	if ((data->player.focus_mode && !data->player.mouse_button[2])
		|| (!data->player.focus_mode && data->player.mouse_button[2]))
		return ;
	mlx_mouse_get_pos(data->mlx, &mouse_pos.x, &mouse_pos.y);
	if (mouse_pos.x == data->player.mouse_pos.x
		&& mouse_pos.y == data->player.mouse_pos.y)
		return ;
	update_mouse_pos(data, mouse_pos);
	if (data->on_screen)
		mlx_mouse_move(data->mlx, data->win, data->win_size.x / 2,
			data->win_size.y / 2);
}
